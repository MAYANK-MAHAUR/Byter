...
export function buildByterAgentSpec(config: TrueForgeRuntimeConfig) {
  return {
    ...
    instructions: [
      ...
      "MANDATORY SANDBOX EXECUTION: You MUST execute the reproducer in the sandbox. Static code inspection alone is strictly prohibited as a basis for verification or submitting a final result. You must run the actual code in the sandbox to observe and record executable stdout/stderr evidence before proposing a patch or concluding.",
...
export function buildInitialUserMessage(input: StartByterSessionInput): string {
  return [
    ...
    "",
    "Required proof path:",
    "1. Triage the claim by reading issue-relevant source files via GitHub MCP.",
    "2. MANDATORY SANDBOX EXECUTION: Launch the sandbox and execute the reproducer. Do NOT stop at static inspection. Run the code in the sandbox to observe and capture concrete before/after stdout evidence.",
    "3. Execute the smallest supplied reproducer in the sandbox before any broad dependency installation.",
    "3a. When that reproducer reaches the target failure, repeat the exact command immediately until 3/3 attempts are recorded before running package tests or installing more dependencies.",
    ...
    "6. Run the fix in the sandbox to confirm it passes and all regression tests pass.",
    "7. After submitting a patch-ready proof, call create_fix_pull_request with the repository owner/name, base branch, reserved fix branch, candidate title/body, and exact candidate files. TrueForge will pause this call for maintainer approval.",
