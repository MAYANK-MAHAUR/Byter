...
export function buildReproSmithAgentSpec(config: TrueForgeRuntimeConfig) {
  return {
    ...
    instructions: [
      ...
      "Run the smallest reproducer supplied by the issue before installing the whole workspace. Do not run workspace-wide install, build, or test commands when a focused Node or package-level reproducer is sufficient. If dependencies are required, use a bounded install with low concurrency and report the environment as blocked when it times out; do not retry the same stalled command indefinitely.",
