...
export function buildInitialUserMessage(input: StartReproSmithSessionInput): string {
  return [
    ...
    "3. Detect the repository language and required runtime from focused manifests; do not assume Node.js.",
    "4. If the runtime or package manager is missing, install a bounded portable user-space toolchain under /tmp/reprosmith-tools and add it to PATH. Use the smallest official runtime needed for this issue; report blocked if that cannot be done safely.",
    "5. Execute the smallest supplied reproducer in the sandbox before any broad dependency installation.",
    "6. Require the same target failure 3/3 before verification.",
    "7. Prepare a complete candidatePatch with exact final file contents if the fix is verified.",
    "8. Pause before any GitHub mutation.",
    "9. Finish with exactly one fenced JSON object using this shape:",
